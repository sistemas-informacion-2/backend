...
export class DisponibilidadService {
  ...
  /**
   * Devuelve un mapa idVariante -> unidades; las variantes sin stock vendible
   * no aparecen (equivalen a 0). Con `idSucursal` suma solo los almacenes de
   * esa sucursal (lo que el cliente puede comprar ahí realmente); sin él,
   * suma en todas las sucursales donde el producto está activado.
   */
  async stockPorVariante(idsVariante: number[], idSucursal?: number): Promise<Map<number, number>> {
