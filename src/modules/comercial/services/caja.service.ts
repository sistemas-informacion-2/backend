...
export class CajaService {
  ...
  async cerrar(id: number, dto: CerrarCajaDto, usuario: ActiveUser): Promise<CajaResponseDto> {
    const caja = await this.cajaRepo.findByIdConDetalle(id);
    if (!caja) throw new NotFoundException('Caja no encontrada');
    await this.validarAccesoSucursal(usuario, caja.idSucursal);
  ...
  /** El administrador opera cualquier sucursal; un empleado solo las que tiene asignadas (EMPLEADO_SUCURSAL). */
  private async validarAccesoSucursal(usuario: ActiveUser, idSucursal: number): Promise<void> {
    if (usuario.tipoUsuario !== 'E') return;
    const asignacion = await this.empleadoSucursalRepo.findOne({
      where: { idEmpleado: usuario.sub, idSucursal, activo: true },
    });
    if (!asignacion) throw new ForbiddenException('No estas asignado a la sucursal de esta caja');
  }
